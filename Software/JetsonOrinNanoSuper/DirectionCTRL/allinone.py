...
- 제공된 모든 코드 파일의 핵심 기능을 통합 및 최적화했습니다.
"""
import asyncio
import logging
import serial
...
# Pydantic이 설치되지 않았을 경우를 대비한 처리
try:
    from pydantic import BaseModel, Field, ValidationError
except ImportError:
    print("Pydantic 라이브러리가 필요합니다. 'pip install pydantic'으로 설치해주세요.")
    exit()

# MAVSDK가 설치되지 않았을 경우를 대비한 처리
try:
    from mavsdk import System
except ImportError:
    print("MAVSDK 라이브러리가 필요합니다. 'pip install mavsdk'으로 설치해주세요.")
    exit()
...
class UnifiedUARTComm:
    ...
    def send_data(self, data_type: str, payload: dict):
        ...
        try:
            ...
            # 복잡한 MAVSDK 객체를 JSON으로 변환하기 위한 default 핸들러 추가
            json_str = json.dumps(message, default=lambda o: o.__dict__ if hasattr(o, '__dict__') else str(o)) + '\n'
...
class ControlCommand(BaseModel):
    ...
    horizontal: Literal["hover"] = "hover" # 수직 테스트이므로 hover로 고정
    speed: float = Field(0.0, ge=0, le=100, description="추력 (%)")
...
async def main():
    ...
    # SITL: "udp://:14540", 실제 FC: "serial:///dev/ttyAMA0:921600" 등
    connection = DroneConnection("udp://:14540")
    # 실제 Jetson UART 포트: "/dev/ttyTHS0" 또는 "/dev/ttyTHS1" 등
    uart = UnifiedUARTComm("/dev/tty.usbmodem14101") # macOS 테스트용
    ...
    async def communication_loop():
        ...
        while True:
            ...
            if connection.latest_telemetry:
                uart.send_data("TELEMETRY", connection.latest_telemetry)
            
            # 2. UART로부터 원격 제어 명령 수신
            raw_cmd = uart.get_command()
            if raw_cmd:
                try:
                    cmd_data = json.loads(raw_cmd).get('command', {})
                    command_obj = ControlCommand(**cmd_data)
                    logging.info(f"유효한 원격 명령 수신: {command_obj.model_dump()}")
    ...
    try:
        ...
        if controller.is_offboard:
            ...
            await connection.drone.action.land()
