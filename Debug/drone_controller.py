...
class DroneController:
    ...
    def takeoff(self, altitude=None):
        ...
        while time.time() - start_time < timeout:
            # 수정: VFR_HUD 대신 GLOBAL_POSITION_INT의 relative_alt 사용
    ...
    def read_altitude(self):
        """현재 상대 고도를 읽어서 반환"""
        # 수정: 이미 GLOBAL_POSITION_INT의 relative_alt를 사용중이므로 변경 없음
        msg = self.master.recv_match(type='GLOBAL_POSITION_INT', blocking=True, timeout=1)
        if msg:
            altitude = msg.relative_alt / 1000.0  # mm를 m로 변환
    ...
    def goto_home(self):
        ...
        while True:
            ...
            # GPS 오류가 지속되면 RTL 모드로 전환
            if distance > 500:  # 500m 이상 벗어난 경우 비정상
